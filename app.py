...
def draw_wheel(angle=0):
    ...
    wedges, _ = ax.pie([1]*num_segments, colors=colors, startangle=angle, counterclock=False, wedgeprops={'edgecolor': 'black'})
    ...
    ax.add_patch(plt.Circle((0, 0), 0.05, color='black'))  # Center circle
    return fig
...
def spin_wheel():
    ...
    for _ in range(steps):
        ...
        wheel_placeholder.pyplot(draw_wheel(current_angle))
        time.sleep(0.03)
    
    # Determine the winning segment
    final_angle = current_angle % 360
    winning_index = int((num_segments - (final_angle // (360 / num_segments))) % num_segments)
